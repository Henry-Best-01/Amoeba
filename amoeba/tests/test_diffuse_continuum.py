...
def test_initialization():
    ...
    assert my_continuum.cloud_density_radial_dependence == cloud_density_radial_dependence
    assert np.shape(my_continuum.cloud_density_array) == np.shape(radii_array)
    assert my_continuum.Om0 == Om0
...
def test_set_responsivity_constant():
    ...
    my_continuum.set_responsivity_constant(
        responsivity_constant = const_a
    )
    ...
    with pytest.raises(AssertionError):
        my_continuum.set_responsivity_constant(
            responsivity_constant = -1
        )
        my_continuum.set_responsivity_constant(
            responsivity_constant = 33
        )
...
def test_interpolate_spectrum_to_wavelength():
    ...
    val_50 = my_continuum.interpolate_spectrum_to_wavelength(
        fifty_percentile
    )

    val_100 = my_continuum.interpolate_spectrum_to_wavelength(
        hundred_percentile
    )
    ...
    ) == my_continuum.interpolate_spectrum_to_wavelength(
        150 * (1 + redshift_source)
    )
...
def test_get_diffuse_continuum_emission():
    ...
    assert my_projection.rest_frame_wavelength_in_nm == wavelength_obs / (1 + redshift_source)
...
def test_get_diffuse_continuum_mean_lag():
    ...
    brute_force_continuum_lag = brute_continuum.get_diffuse_continuum_mean_lag(wavelength_obs)
...
def test_get_diffuse_continuum_lag_contribution():
    ...
    diffuse_continuum_additional_lag = my_continuum.get_diffuse_continuum_lag_contribution(wavelength_obs)
    ...
    middle_wavelength = minimum_delay_wavelength + (longest_delay_wavelength - minimum_delay_wavelength) / 2
    quarter_wavelenth = minimum_delay_wavelength + (middle_wavelength - minimum_delay_wavelength) / 2
    ...
    longest_delay = my_continuum.get_diffuse_continuum_lag_contribution(longest_delay_wavelength)
    minimum_delay = my_continuum.get_diffuse_continuum_lag_contribution(minimum_delay_wavelength)
    middle_delay = my_continuum.get_diffuse_continuum_lag_contribution(middle_wavelength)
    quarter_delay = my_continuum.get_diffuse_continuum_lag_contribution(quarter_wavelenth)
