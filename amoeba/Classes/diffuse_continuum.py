...
class DiffuseContinuum:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if cloud_density_array is not None:
            ...
            self.cloud_density_array = (self.radii_array ** cloud_density_radial_dependence) * self.radial_mask
            self.cloud_density_radial_dependence = cloud_density_radial_dependence
        else:
            raise ValueError("Cloud density array and cloud density radial dependence cannot both be None type")
    ...
    def set_responsivity_constant(
        self,
        responsivity_constant = None
    ):
        """
    ...
    def interpolate_spectrum_to_wavelength(
        ...
        rest_frame_wavelength_in_nm = observer_frame_wavelength_in_nm / (1 + self.redshift_source)
    ...
    def get_diffuse_continuum_emission(
        self,
        observer_frame_wavelength_in_nm,
        incident_continuum_weighting=1
    ):
        """
        ...
        """
        rest_frame_wavelength_in_nm = observer_frame_wavelength_in_nm / (1 + self.redshift_source)
        emission_array = self.cloud_density_array * np.nan_to_num(self.radii_array**(-2))
        emission_array /= np.sum(emission_array)
        emission_array *= self.interpolate_spectrum_to_wavelength(rest_frame_wavelength_in_nm) * incident_continuum_weighting
    ...
    def get_diffuse_continuum_mean_lag(
        self,
        observer_frame_wavelength_in_nm
    ):
        """
        ...
        if self.cloud_density_radial_dependence is not None:
            ...
            if self.cloud_density_radial_dependence < 0:
                integration_multiplicative_constant = (2 * np.pi) / (self.cloud_density_radial_dependence)
                ...
                return integration_multiplicative_constant * np.log(
                    self.r_out_in_gravitational_radii/self.r_in_in_gravitational_radii
                ) + integration_constant
            ...
            time_delays = calculate_time_lag_array(self.radii_array, self.phi_array, 0, 0)
            emissivity = self.get_diffuse_continuum_emission(observer_frame_wavelength_in_nm).flux_array
            ...
            diffuse_continuum_transfer_function /= np.sum(diffuse_continuum_transfer_function)

            mean_delay = np.sum(np.linspace(0, np.max(time_delays)-1, int(np.max(time_delays))) * diffuse_continuum_transfer_function)
